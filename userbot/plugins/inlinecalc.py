...
@catub.cat_cmd(
    ...
    info={
        ...
        "usage": "{tr}icalc",
        "description": "An awesome inline calculator with cool buttons. Ported from Ultroid by @realnub.",
    },
)
async def icalc(e):
    if e.client._bot:
        return await e.reply("**𝙄𝙉𝙇𝙄𝙉𝙀 𝘾𝘼𝙏 𝘾𝘼𝙇𝘾𝙐𝙇𝘼𝙏𝙊𝙍**", buttons=lst)
...
async def inlinecalc(event):
    ...
    if (
        ...
        calc = event.builder.article(
            "Calc", text="**𝙄𝙉𝙇𝙄𝙉𝙀 𝘾𝘼𝙏 𝘾𝘼𝙇𝘾𝙐𝙇𝘼𝙏𝙊𝙍**", buttons=lst
        )
...
async def _(e):
    ...
    lst = list(zip(tultd[::4], tultd[1::4], tultd[2::4], tultd[3::4]))
    lst.append([Button.inline("=", data="calc=")])
    await e.edit("**𝙄𝙉𝙇𝙄𝙉𝙀 𝘾𝘼𝙏 𝘾𝘼𝙇𝘾𝙐𝙇𝘼𝙏𝙊𝙍**", buttons=lst)
