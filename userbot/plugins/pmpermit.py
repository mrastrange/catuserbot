...
async def do_pm_permit_action(event, chat):  # sourcery no-metrics
    ...
    try:
        MAX_FLOOD_IN_PMS = int(gvarstatus("MAX_FLOOD_IN_PMS") or 2)
...
**Spamming Detected! 
Blocked! 
Stop spamming here re , without common sense. GTFO and SPAM somewhere else 😒
Assistant: @ZypherWiperBot**"""
...
async def do_pm_enquire_action(event, chat):
    ...
    try:
        ...
        PMMESSAGE_CACHE = {}
    if str(chat.id) not in PM_WARNS:
        text = """__Hey! Have some patience. @ZypherWiper has not seen your message yet. \
...
async def on_plug_in_callback_query_handler(event):
    ...
    text = "Ok, Now you are accessing the availabe menu of @ZypherWiper . \\\x1f__Let's make this smooth and let me know why you are here.__\x1f\x1f**Choose one of the following reasons why you are here:**"
    ...
    buttons = [
        ...
        (
            Button.inline(
                text="To spam @ZypherWiper's inbox.",
...
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        ...
        return await event.answer(text, cache_time=0, alert=True)
    text = """__Okay. Your request has been registered. Do not spam @ZypherWiper's inbox now. \
...
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "Idoit this options for user who messages you. not for you"
        return await event.answer(text, cache_time=0, alert=True)
    text = """__Okay. I have notified @ZypherWiper about this. When he comes comes online\
...
async def on_plug_in_callback_query_handler(event):
    ...
    text = "**So uncool, this is not your home. Go bother somewhere else.\
         \n\nRemember that this is your last warning and if you send one more message you will be blocked automatically.**"
...
async def block_p_m(event):
    ...
    await edit_delete(
        event,
        f"[{user.first_name}](tg://user?id={user.id}) __is blocked, he/she can no longer personal message you.__\n**Reason:** __{reason}__",
    )
